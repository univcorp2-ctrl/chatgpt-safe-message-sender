...
- `make verify`、`make report`、`tox`、`docker build` による GitHub Actions 非依存の検証
...
docker compose run --rm safe-message-sender
```
...
- `outputs/live-smoke-response.json`

詳しくは [`docs/test-matrix.md`](docs/test-matrix.md) を参照してください。
